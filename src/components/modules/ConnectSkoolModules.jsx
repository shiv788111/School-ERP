...
function Dot({ status }) {
  ...
  return <span className={`inline-block w-2.5 h-2.5 rounded-full ${cfg.bg} shadow-sm`} title={cfg.label} />;
}
  ...
  return (
    <div id={`section-${mod.id}`} className="mb-8 scroll-mt-20">
      ...
      <div className="flex items-center gap-2.5 mb-4 pb-1 border-b border-amber-200/50">
        ...
        <h2 className="text-base font-bold text-[#f0970a] tracking-tight">{mod.label}</h2>
        <span className="text-[11px] font-medium text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full ml-2">{mod.features.length}</span>
      </div>
      ...
      <div className="rounded-xl overflow-hidden border border-amber-100 bg-white shadow-sm">
        ...
        {mod.features.map((feat, i) => (
          ...
          >
            ...
            <div className="px-3 py-2.5 flex items-center justify-center">{/* <Dot status={feat.other} /> */}</div>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="min-h-screen bg-[#F7F8FB] font-['Inter',system-ui,sans-serif]">
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
        <div className="flex gap-6">
          ...
          >
            <div className="p-3">
              <div className="text-[10px] font-bold uppercase tracking-wider text-gray-400 px-3 pt-2 pb-3">Module Names</div>
          ...
          <main ref={contentRef} className="flex-1 overflow-y-auto h-[calc(100vh-120px)] pr-2" style={{ scrollBehavior: 'smooth' }}>
